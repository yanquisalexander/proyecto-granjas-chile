...
class Role {
  ...
  static async addRole(user: User, role: RoleValue): Promise<void> {
    await Database.query('INSERT INTO user_roles (user_id, role_id) VALUES ($1, (SELECT id FROM roles WHERE name = $2))', [user.id, role])
  }

  static async removeRole(user: User, role: RoleValue): Promise<void> {
  ...
  static async createInitialRoles(): Promise<void> {
    ...
    for (const role of roles) {
      await dbService.insert(RolesTable)
        .values({
          name: role
        })
        .onConflictDoNothing()
