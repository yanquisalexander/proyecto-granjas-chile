...
import { z } from 'zod'
import Role, { RoleValue, Roles } from './Role.model'
...
class User {
  ...
  async hasRole(roles: RoleValue[]): Promise<boolean> {
    const userRoles = await this.getRoles();
    return userRoles.some(userRole => roles.includes(userRole.name as RoleValue))
  }
  ...
  async addRole(role: RoleValue): Promise<void> {
    await Role.addRole(this, role as RoleValue)
  }

  async removeRole(role: RoleValue): Promise<void> {
    await Role.removeRole(this, role as RoleValue)
  }
  ...
  static async findByEmailOrUsername(email: string, username: string): Promise<UserDTO | null> {
    ...
    const roles = await dbService.query.UserRolesTable.findMany({
      where: eq(UserRolesTable.user_id, result.id),
      with: {
        role: true
      }
    })
